...
export const FavRender = ({ info }) => {
  const dispatch = useDispatch();
  return (
    <div className=" relative pd-2 m-2 shadow-xl rounded-xl overflow-y-hidden h-[550px]">
      <PlanetCard info={info} />
      <div className=' '>
        <div
          onClick={() => {
          ...
          className="p-2 m-2 flex shadow-sm cursor-pointer absolute inset-x-0 bottom-0"
        >
          <img
            className="h-10 w-10 p-2 rounded-xl mr-3"
            alt="delete"
          ...
          />
          <h1 className="p-2">Remove this item</h1>
        </div>
      </div>
    </div>
  );
